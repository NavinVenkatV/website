...
const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  ({ className, type, ...props }, ref) => {
    ...
    const mouseX = useMotionValue(0); // Use const instead of let
    const mouseY = useMotionValue(0); // Use const instead of let

    function handleMouseMove(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
      const { left, top } = e.currentTarget.getBoundingClientRect(); // Use const for left and top
